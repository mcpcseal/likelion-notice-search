...
2. 게시판별로 `pageIndex=1`부터 시작해 목록 페이지를 순회하며 `nttId` 목록 추출 (`MAX_PAGES`=20 안전장치 — 전체 히스토리 재크롤링 방지)
   - 페이지에 글이 0건이면 더 이상 페이지가 없는 것이므로 순회 종료
   - 페이지의 모든 글이 이미 저장된 상태면(= 최신 글까지 다 따라잡음) 그 이후 페이지는 더 오래된 글만 있을 것이므로 순회 종료
